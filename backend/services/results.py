...
def get_results(username: str, admin_view: bool):
    with get_connection() as conn:
        ...
        query = """
                    ...
                    FROM exam_catalog AS t1
                    JOIN exam_attempts AS t2
                        ON t1.exam_id = t2.exam_id
                    WHERE t2.submission_time IS NOT NULL
                    """
        if not admin_view:
        ...
        #for cases when 0 question attempts were made, SQL would return NULL -> fix:score=result[8] or 0
        return [ExamResult(attempt_id=result[0], exam_id=result[1], name=result[2], exam_type=result[3],
                           ...
                           score=result[8] or 0, submission_time=result[9], username=result[10], 
                           evaluation_status=result[11]) for result in results]
...
def get_attempt_result(username: str, attempt_id: str, is_admin: bool):
    with get_connection() as conn:
        ...
        evaluation_status = exam_result[10]
        ...
        #for cases when 0 question attempts were made, SQL would return NULL -> fix:score=result[7] or 0
